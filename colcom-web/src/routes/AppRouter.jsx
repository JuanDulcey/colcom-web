...
import { ForgotPasswordPage } from '../pages/admin/ForgotPasswordPage.tsx';
...
export function AppRouter() {
  ...
  const { setCountryBySlug, loading, activeSlug } = useCountry();
  const reservedPaths = new Set(['admin', 'noticias', 'testimonios', 'contacto']);
  ...
  if (path === '/admin/login' || path === '/login') return <LoginPage />;
  if (path === '/recuperar' || path === '/forgot-password') return <ForgotPasswordPage />;
  if (segments[1] === 'login' && !reservedPaths.has(segments[0])) {
    return <LoginPage />;
  }

  const getWidgetCountry = (slug) => {
    switch(slug) {
      case 'colombia': return 'co';
      case 'chile': return 'cl';
      case 'argentina': return 'ar';
      case 'ecuador': return 'ec';
      default: return 'latam';
    }
  };

  const widgetCountry = getWidgetCountry(activeSlug);
  ...
  return (
    <>
      <PublicLayout>{publicPage}</PublicLayout>
      <ChatWidget country={widgetCountry} title="Asistente Virtual" placeholder="Escribe tu mensaje..." />
    </>
  );
}
